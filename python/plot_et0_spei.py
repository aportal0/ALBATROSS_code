...
def build_title_combo(method, month, window, year):
    ...
    return f"ET0-mean, SPEI-{window}, P-sum, WB-sum - {method_label}\n{window_label}, {period}"
...
def plot_et0_spei_pr_wb(et0, spei, pr, wb, method, month, window, year, outpath):
    ...
    _format_map(axes[0, 0], f"ET0 ({window}-month mean)")
    ...
    _format_map(axes[0, 1], f"SPEI-{window}")
    ...
    _format_map(axes[1, 0], f"P ({window}-month cum.)")
    ...
    fig.colorbar(im0, ax=axes[0, 0], label="ET0 (mm/day)", shrink=0.8, extend="max")
    fig.colorbar(im1, ax=axes[0, 1], label=f"SPEI-{window} (-)", shrink=0.8, extend="both")
    fig.colorbar(im2, ax=axes[1, 0], label="P (mm)", shrink=0.8, extend="max")
    fig.colorbar(im3, ax=axes[1, 1], label="WB (mm)", shrink=0.8, extend="both")
